...
$(function () {
    SetClearWord("input.clear_word,textarea.clear_word,textarea.text_area");
});
...
$(function () {
    ...
    $(".nsw_pro_list tbody").delegate('span.ck_box', 'click', function () {
    ...
    $('.f_mlqx').delegate('span', 'click', function () {
       var me = $(this);
        ...
        if (input.attr("checked")) {
            ...
            $(".fancyTable tbody").find(".ck_box").find("input").prop("checked", true);
            $(".fancyTable tbody").find(".ck_box").addClass("z_select");
        } else {

            $(".fancyTable tbody").find(".ck_box").find("input").prop("checked", false);
            $(".fancyTable tbody").find(".ck_box").removeClass("z_select");
        }
        if (input.attr("checked")) {

            $("#sptbd").find(".ck_box").find("input").prop("checked", true);
            $("#sptbd").find(".ck_box").addClass("z_select");
        } else {

            $("#sptbd").find(".ck_box").find("input").prop("checked", false);
    ...
    var ShowImg_ = function (img) {
        ...
        loadCss("skins/fancybox/jquery.fancybox.css", window.top.document);
        loadJs("js/easyui/jquery.fancybox.js", function () {
            window.top.$.fancybox.open([{ href: img.attr("src"), title: img.attr("title") }]);
        }, window.top.document);
    }
    var close_ = function () {
    ...
    //    layer.open({
    //        title: '批量修改交期时间',
    //        content: '输入时间:<input  class="Wdate" onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\',readOnly:true})" type="text" id="yyyo"/>',
    //        btn: ['确定', '关闭'],
    ...
    $("#jiagong .xuanz_ul").delegate('li', 'click', function () {
        var xuanze = $(this).find("a").html();
        $("#jiagong .xuanz_span").attr("values", $(this).find("a").attr("values"));
        $("#jiagong .xuanz_span").html(xuanze);
    });
    $("#pinpai .xuanz_ul").delegate('li', 'click', function () {
        var xuanze = $(this).find("a").html();
    ...
    //添加季度筛选框
    $("#jidu .xuanz_ul").delegate('li', 'click', function () {
        var xuanze = $(this).find("a").html();
        $("#jidu .xuanz_span").attr("values", $(this).find("a").attr("values"));
        $("#jidu .xuanz_span").html(xuanze);
    });

    /*table的头部滚动时漂浮*/

    //$(".spxd_gun").scroll(function () {

    //    //$(".thead1").scrollLeft = $(this).scrollLeft;
    //    theaddiao();
    //});
    //function theaddiao() {

    //    var thead1 = $(".fht-thead").find("thead tr th");
    //    var spxdgun = $(".fht-tbody").find("thead tr th");
    //    //var thead1tr = $(".fht-thead").find("thead tr");
    //    //var spxdguntr = $(".fht-tbody").find("thead tr");
    //    //var wids = spxdguntr.width();
    //    //thead1tr.css({ "width": wids });
    //    //$(".thead1").css({ "left": " 0", "position": "absolute", "top": " 0","width": "99%", "z-index": " 999" }).show();
    //    //alert(spxdgunss.width());
    //    var wid = "";
    //    spxdgun.each(function (indexTh) {
    //        wid += spxdgun.eq(indexTh).width() + ",";
    //        var pp = wid.substring(0, wid.length - 1).split(',');
    //        thead1.each(function (indexTh1) {
    //            thead1.eq(indexTh1).css({ "width": pp[indexTh1] });
    //        });
    //    });
    //    console.log(thead1.eq(indexTh1).width());
    ...
    $(function () {
        $(".yijian1").click(function () {

            var table_yj = $("td[data-id='" + $(this).attr("data-id") + "']");
            var frl_span = $("td[data-id='" + $(this).attr("data-id") + "']").find(".frl_span");


            if (frl_span.next("span").hasClass("table_yj_show")) {

                frl_span.next("span").removeClass("table_yj_show");
                frl_span.show();
                table_yj.find(frl_span).next("span").hide();
                EditOpinion()//修改意见
            } else {
            ...
            if ($(this).val() == "项目意见修改") {
                ...
                $(this).val("项目意见确定");
            } else if ($(this).val() == "生管意见修改") {

                $(this).val("生管意见确定");
            } else if ($(this).val() == "综合评定是否合作修改") {
                $(this).val("综合评定是否合作确定");
            } else if ($(this).val() == "项目意见确定") {
                $(this).val("项目意见修改");
            } else if ($(this).val() == "生管意见确定") {
                $(this).val("生管意见修改");
            } else {
                $(this).val("综合评定是否合作修改");
            }
    ...
    function cutStr(len) {
        var obj = $("#bdy_da tr").find(".bzclick");
    ...
    //备注显示
    function bzclick(obj) {
        var objsss = $(obj).parent().find(".bzclick1");
        $(".beiz").show();
        $("#maskIframe").show();
        $("#beiz").val(objsss.html());
    ...
    function addtr() {
        ...
        $.get("/GoodsInfo/GetDDLSCJD02", {}, function (data) {
            var ddlBD = "<select class='xjym_spinp'>";
            var dt = eval("(" + data + ")");
            ...
            var tr = '<tr>';
            tr += ' <td>' + ddlBD + '</td>';
            tr += ' <td> <select class="xjym_spinp"><option selected="selected" value="Banana Baby">Banana Baby</option><option value="Banana CiCi">Banana CiCi</option><option value="Banana Kids">Banana Kids</option><option value="E-commerce">E-commerce</option></select></td>';
            tr += ' <td> <select class="xjym_spinp"><option selected="selected" value="未定义">未定义</option><option value="CMT">CMT</option><option value="FOB">FOB</option></select></td>';
            tr += ' <td><span class="sspa"></span><input id="txtkuanshi" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtkuanhao" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtcolor" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtmax" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtmal2" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtmal3" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtmaxl" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input id="txtjunma" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input onfocus="WdatePicker({dateFmt:\'yyyy-MM-dd\',readOnly:true})" id="txtjiaoq" type="text" class="xjym_spinp" name=""></td>';
            tr += ' <td><span class="sspa"></span><input onfocus="WdatePicker({dateFmt:\'yyyy\',readOnly:true})" id="txtnianf" type="text" class="xjym_spinp" name=""></td>';
            tr += '<td class="td_bnt"> <div  onclick="addtr()"  class="xj_bnt xj_bnt1" > <span class="bnt_span1"><img class="bnt_img" src="/skins/img/xj.png" style=""></span>新建 </div><div class="xj_bnt xj_bnt2" onclick="deltr(this)"> <i class="ace-icon fa fa-trash-o bigger-120 orange"></i>删除 </div></td>';
            tr += '</tr>'
            $(".sp_xj tbody").append(tr);
        })
...
//供应商双击输入框弹出的窗口
    function queren() {
        ...
        var daima = $(".ad_cls").find("td").eq(0);
        var daima_name = $(".ad_cls").find("td").eq(1);
        $("#GYSGC50").val(daima.find("span").html());
        $("#GYSGC03").val(daima_name.find("span").html());
        $("#txtGirard").val(daima_name.find("span").html());
        deleteGlobalShade();
    }
    $(function () {
        $('.bntxy').click(function () {
            $('.ld_hid').toggle(100);
        });
        ...
        $("#GYSGC50").dblclick(function () {
            $(".Reconciliation_ser_wai").show(100);
            $("#maskIframe").show(100);
        });
        
        $("#Reconciliation_ser_wai1 .Reconciliation_ser").delegate("td", "click", function () {
        ...
        $(" #Reconciliation_ser_wai1 tbody").delegate(".short_tit", "dblclick", function () {
            queren();
   ...
   function djbeijin() {
       ...
       a.hover(function () {
          a.eq($(this).index()).css("background", "#FFE2C3");
           $("#sptbd tr").eq($(this).index()).css("background", "#FFE2C3");

       },function () {
           a.eq($(this).index()).css("background", "none");
           $("#sptbd tr").eq($(this).index()).css("background", "none");
           $(".child_1 ").css("background", "#dbe3e9");
           $(".child_2 ").css("background", " #fff");
         });
   }
