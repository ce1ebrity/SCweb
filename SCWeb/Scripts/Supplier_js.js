        ...
        data:
            {
                ...
                GYSGC01: $("#boduan").find("span").attr("values"),
            ...
            if (data == "error") {
                ...
                if (dt.length>0) {
                    for (var i = 0; i < dt.length; i++) {
                        ...
                        tableHtml += "<td style='width: 90px;'><span class='poducts short_tit f_fl tdtop'><a href='##' class='update_Title' >" + dt[i].GYSGC01 + "</a><i class='e_edi1 e_more_edit popUp' style='display: none;' onclick='btnEdit(" + dt[i].id + ")'></i></span></td>";
                        tableHtml += "<td style='width: 90px;'><span class='poducts short_tit f_fl'>" + dt[i].GYSGC02 + "</span></td>";
                        ...
                        tableHtml += "<td style='width: 90px;' class='nsw_cnt_area action_tx'><a onclick='btnEdit(" + dt[i].id + ")'><span class='pro_view pro_edit'><i></i>编辑</span></a></td>";
                        tableHtml += "</tr>";
                    }
...
function GetDDLOP() {
    $("#gysTB").find("tr").each(function (index) {
        ...
        var GYSGC13 = $(this).find("td").eq(14).find("span").eq(0).html();
        var GYSGC14 = $(this).find("td").eq(15).find("span").eq(0).html();
        var GYSGC15 = $(this).find("td").eq(16).find("span").eq(0).html();
        var GYSGC16 = $(this).find("td").eq(17).find("span").eq(0).html();
        var GYSGC17 = $(this).find("td").eq(18).find("span").eq(0).html();
        var GYSGC18 = $(this).find("td").eq(19).find("span").eq(0).html();
        var GYSGC19 = $(this).find("td").eq(20).find("span").eq(0).html();
        //下拉框绑定值
        $(this).find("td").eq(14).find("select").val(GYSGC13);
        $(this).find("td").eq(15).find("select").val(GYSGC14);
        $(this).find("td").eq(16).find("select").val(GYSGC15);
        $(this).find("td").eq(17).find("select").val(GYSGC16);
        $(this).find("td").eq(18).find("select").val(GYSGC17);
        $(this).find("td").eq(19).find("select").val(GYSGC18);
        $(this).find("td").eq(20).find("select").val(GYSGC19);
...
function GetSpanByDDLOP() {
    $("#gysTB").find("tr").each(function (index) {
        ...
        //获取下拉框绑定值
        //var thArr = $(this).children();
        var GYSGC13 = $(this).find("td").eq(14).find("select").val();
        var GYSGC14 = $(this).find("td").eq(15).find("select").val();
        var GYSGC15 = $(this).find("td").eq(16).find("select").val();
        var GYSGC16 = $(this).find("td").eq(17).find("select").val();
        var GYSGC17 = $(this).find("td").eq(18).find("select").val();
        var GYSGC18 = $(this).find("td").eq(19).find("select").val();
        var GYSGC19 = $(this).find("td").eq(20).find("select").val();

        //将值付给Span
        $(this).find("td").eq(14).find("span").eq(0).html(GYSGC13);
        $(this).find("td").eq(15).find("span").eq(0).html(GYSGC14);
        $(this).find("td").eq(16).find("span").eq(0).html(GYSGC15);
        $(this).find("td").eq(17).find("span").eq(0).html(GYSGC16);
        $(this).find("td").eq(18).find("span").eq(0).html(GYSGC17);
        $(this).find("td").eq(19).find("span").eq(0).html(GYSGC18);
        $(this).find("td").eq(20).find("span").eq(0).html(GYSGC19);
...
function EditOpinion() {
    ...
    $("#gysTB").find("tr").each(function (index) {
        ...
        var GYSGC13 = $(this).find("td").eq(14).find("select").val();//项目意见配合度
        var GYSGC14 = $(this).find("td").eq(15).find("select").val();//项目意见开发能力
        var GYSGC15 = $(this).find("td").eq(16).find("select").val();//项目意见是否可配合
        var GYSGC16 = $(this).find("td").eq(17).find("select").val();//生管意见配合度
        var GYSGC17 = $(this).find("td").eq(18).find("select").val();//生管意见货期
        var GYSGC18 = $(this).find("td").eq(19).find("select").val();//生管意见是否可配合
        var GYSGC19 = $(this).find("td").eq(20).find("select").val();//综合评定是否合作
...
//编辑前绑定数据
function btnEdit(id) {
    location.href = "/Supplier/SupplierAddUpdate/" + id;
}
...
$(function () {
    ...
    //获取值绑定产品类别下拉框
    $.get("/Supplier/GetDDLType", { LBid: "1" }, function (data) {
        var ddlType = "";
        var dt = eval("(" + data + ")");
        for (var i = 0; i < dt.length; i++) {
    ...
    $("#lnkBtnDel").click(function () {
        ...
        if (ckval == "") {
            ...
            layer.open({
                ...
                btn: ['确定', '取消'],
                yes: function (index, layero) {
                    $.get("/Supplier/DelGYSGCGL", { id: ckval }, function (data) {
                        if (data == "success") {
                            layer.open({
                                title: '消息',
                                content: '删除成功!',
